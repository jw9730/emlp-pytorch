...
class ParticleInteraction(object):
    ...
    def __init__(self, N=1024):
        ...
        def dot(v1, v2): return ((v1@𝜂)*v2).sum(-1)
        Le = (p1[:, :, None]*p3[:, None, :] -
              (dot(p1, p3)-dot(p1, p1))[:, None, None]*𝜂)
        L𝜇 = ((p2@𝜂)[:, :, None]*(p4@𝜂)[:, None, :] -
              (dot(p2, p4)-dot(p2, p2))[:, None, None]*𝜂)
        ...
        # does not violate equivariance
        self.Xscale = np.sqrt(
            (np.abs((self.X.reshape(N, 4, 4)@𝜂)*self.X.reshape(N, 4, 4)).mean(-1)).mean(0))
        self.Xscale = (self.Xscale[:, None]+np.zeros((4, 4))).reshape(-1)
        self.stats = 0, self.Xscale, self.Y.mean(axis=0), self.Y.std(
            axis=0)  # self.X.mean(axis=0),self.X.std(axis=0),
...
class GroupAugmentation(Module):
    def __init__(self, network, rep_in, rep_out, group):
        ...
        self.rho_in = jit(vmap(lambda g: self.rep_in.rho(g).to_dense()))
        self.rho_out = jit(vmap(lambda g: self.rep_out.rho(g).to_dense()))
        self.model = network
...
def ULBcorner_rot(state, i=1):
    ...
    ULB_corner_ids = np.array(
        [0, 4*8, 3*8+2])  # top left of U, top left of L, top right of B
    rotated_corner_state = state.copy()
    rotated_corner_state[:, ULB_corner_ids] = rotated_corner_state[:, np.roll(
        ULB_corner_ids, i)]
