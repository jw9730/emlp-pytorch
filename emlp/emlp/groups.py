    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_orthogonal is None:
            ...
            if len(self.lie_algebra) != 0:
                A_dense = jnp.stack([Ai@jnp.eye(self.d)
                                    for Ai in self.lie_algebra])
                self.is_orthogonal &= rel_err(
                    -A_dense.transpose((0, 2, 1)), A_dense) < 1e-6
            if len(self.discrete_generators) != 0:
                h_dense = jnp.stack([hi@jnp.eye(self.d)
                                    for hi in self.discrete_generators])
                self.is_orthogonal &= rel_err(h_dense.transpose(
                    (0, 2, 1))@h_dense, jnp.eye(self.d)) < 1e-6
        ...
        if self.is_orthogonal and (self.is_permutation is None):
            ...
            if len(self.discrete_generators) != 0:
                h_dense = jnp.stack([hi@jnp.eye(self.d)
                                    for hi in self.discrete_generators])
                self.is_permutation &= (
                    (h_dense == 1).astype(int).sum(-1) == 1).all()
...
def matrix_power_simple(M, n):
    ...
    def body(Mn): return jax.lax.fori_loop(0, Mn[1], lambda i, g: Mn[0]@g, out)
...
class RubiksCube(Group):  # 3x3 rubiks cube
    ...
    def __init__(self):
        ...
        order_padded[0, :] = np.rot90(order_padded[0].reshape(
            3, 3), 1).reshape(9)  # Rotate top face
        FRBL = np.array([1, 2, 3, 4])
        order_padded[FRBL, :3] = order_padded[np.roll(
            FRBL, 1), :3]  # F <- L,R <- F,B <- R,L <- B
...
class ZksZnxZn(Group):
    ...
    def __init__(self, k, n):
        ...
        Ik = I(k)
        assert k in [2, 4]
        self.discrete_generators = [LazyKron([Ik, nshift, In]), LazyKron(
            [Ik, In, nshift]), LazyKron([kshift, Rot90(n, 4//k)])]
        super().__init__(k, n)
...
class Embed(Group):
    ...
    def __init__(self, G, d, slice):
        ...
        self.discrete_generators = np.zeros(
            (G.discrete_generators.shape[0], d, d))
