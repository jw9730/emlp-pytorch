...
def product(c): return reduce(lambda a, b: a*b, c)
...
class LazyKron(LinearOperator):
    ...
    def __init__(self, Ms):
        self.Ms = Ms
        shape = product([Mi.shape[0] for Mi in Ms]), product(
            [Mi.shape[1] for Mi in Ms])
    ...
    def _matmat(self, v):
        ...
        for i, M in enumerate(self.Ms):
            ...
            Mev_front = (
                M@ev_front.reshape(M.shape[-1], -1)).reshape(M.shape[0], *ev_front.shape[1:])
    ...
    def to_dense(self):
        Ms = [M.to_dense() if isinstance(M, LinearOperator)
              else M for M in self.Ms]
...
class LazyJVP(LinearOperator):
    def __init__(self, operator_fn, X, TX):
        ...
        self.vjp_T = lambda v: jax.jvp(
            lambda x: operator_fn(x).T@v, [X], [TX])[1]
...
class ConcatLazy(LinearOperator):
    ...
    def to_dense(self):
        dense_Ms = [M.to_dense() if isinstance(
            M, LinearOperator) else M for M in self.Ms]
        return jnp.concatenate(dense_Ms, axis=0)


class LazyDirectSum(LinearOperator):
    def __init__(self, Ms, multiplicities=None):
        self.Ms = [jax.device_put(M.astype(np.float32)) if isinstance(
            M, (np.ndarray)) else M for M in Ms]
        self.multiplicities = [
