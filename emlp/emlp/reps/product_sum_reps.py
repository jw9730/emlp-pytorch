...
class SumRep(Rep):
    def __init__(self, *reps, extra_perm=None):  # repcounter,repperm=None):
        ...
        reps = [SumRepFromCollection({Scalar: rep}) if isinstance(
            rep, int) else rep for rep in reps]
        # Get reps and permutations
        reps, perms = zip(*[rep.canonicalize() for rep in reps])
        rep_counters = [rep.reps if isinstance(rep, SumRep) else {
            rep: 1} for rep in reps]
        ...
        self.is_permutation = all(
            rep.is_permutation for rep in self.reps.keys())
    ...
    def __str__(self):
        tensors = "+".join(f"{count if count > 1 else ''}{rep}" for rep,
                           count in self.reps.items())
    ...
    def equivariant_basis(self):
        ...
        Qs = {rep: (jax.device_put(Q.astype(np.float32)) if isinstance(
            Q, (np.ndarray)) else Q) for rep, Q in Qs.items()}
        active_dims = sum([self.reps[rep]*Qs[rep].shape[-1]
                          for rep in Qs.keys()])
        multiplicities = self.reps.values()
    ...
    def as_dict(self, v):
        ...
        for rep, c in self.reps.items():
            chunk = c*rep.size()
            out_dict[rep] = v[..., self.perm[i:i+chunk]
                              ].reshape(v.shape[:-1]+(c, rep.size()))
            i += chunk
        return out_dict
...
def distribute_product(reps, extra_perm=None):
    ...
    for prod in itertools.product(*[rep.reps.items() for rep in reps]):
        ...
        prod_rep, canonicalizing_perm = (
            product(cs)*reduce(lambda a, b: a*b, rs)).canonicalize()
        ...
        mul_perm = np.arange(len(canonicalizing_perm)).reshape(
            shape).transpose(axis_perm).reshape(-1)
...
def rep_permutation(repsizes_all):
    ...
    size_cumsums = [np.cumsum([0] + [size for size in repsizes])
...
class ProductRep(Rep):
    def __init__(self, *reps, extra_perm=None, counter=None):
        ...
        if counter is not None:  # one with counter specified directly
            ...
            self.reps, self.perm = self.compute_canonical(
                [counter], [np.arange(self.size()) if extra_perm is None else extra_perm])
        else:  # other with list
            # Get reps and permutations
            reps, perms = zip(*[rep.canonicalize() for rep in reps])
            rep_counters = [rep.reps if type(rep) == ProductRep else {
                rep: 1} for rep in reps]
        ...
        assert len(
            Gs) == 1, f"Multiple different groups {Gs} in product rep {self}"
        self.G = Gs[0]
        self.is_permutation = all(
            rep.is_permutation for rep in self.reps.keys())
    ...
    def compute_canonical(rep_cnters, rep_perms):
        ...
        order = order.reshape(tuple(
            rep.size() for cnter in rep_cnters for rep, c in cnter.items() for _ in range(c)))
...
class DirectProduct(ProductRep):
    ...
    def __init__(self, *reps, counter=None, extra_perm=None):
        ...
        if counter is not None:  # one with counter specified directly
            ...
            rep_counters = [rep.reps if type(rep) == DirectProduct else {
                rep: 1} for rep in reps]
            ...
            self.reps, perm2 = self.compute_canonical(
                [counter], [np.arange(self.size())])
            self.perm = extra_perm[perm[perm2]
                                   ] if extra_perm is not None else perm[perm2]
        self.invperm = np.argsort(self.perm)
        self.canonical = (self.perm == self.invperm).all()
    ...
    def equivariant_basis(self):
        canon_Q = LazyKron([rep.equivariant_basis()
                           for rep, c in self.reps.items()])
        return LazyPerm(self.invperm)@canon_Q

    def equivariant_projector(self):
        canon_P = LazyKron([rep.equivariant_projector()
                           for rep, c in self.reps.items()])
