...
class Rep(object):
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        d1 = tuple([(k, v) for k, v in self.__dict__.items() if (
            k not in ['_size', 'is_permutation', 'is_orthogonal'])])
        d2 = tuple([(k, v) for k, v in other.__dict__.items() if (
            k not in ['_size', 'is_permutation', 'is_orthogonal'])])
        return d1 == d2
    ...
    def constraint_matrix(self):
        ...
        n = self.size()
        constraints = []
        constraints.extend([lazify(self.rho(h))-I(n)
    ...
    def equivariant_basis(self):
        ...
        if canon_rep not in self.solcache:
            ...
            logging.info(
                f"Solving basis for {self}"+(f", for G={self.G}" if hasattr(self, "G") else ""))
    ...
    def __pow__(self, other):
        ...
        assert isinstance(
            other, int), f"Power only supported for integers, not {type(other)}"
...
def krylov_constraint_solve(C, tol=1e-5):
    ...
    if C.shape[0]*r*2 > 2e9:
        raise Exception(
            f"Solns for contraints {C.shape} too large to fit in memory")
    found_rank = 5
    while found_rank == r:
...
def krylov_constraint_solve_upto_r(C, r, tol=1e-5, lr=1e-2):  # ,W0=None):
    ...
    W = np.random.randn(C.shape[-1], r) / \
        np.sqrt(C.shape[-1])  # if W0 is None else W0
    W = device_put(W)
    ...
    def loss(W):
        ...
        return (jnp.absolute(C@W)**2).sum()/2
    ...
    for i in range(20000):
        ...
        # update progress bar
        progress = max(100*np.log(lossval/lstart) /
                       np.log(tol**2/lstart)-prog_val, 0)
...
def bilinear_weights(out_rep, in_rep):
    ...
    x_multiplicities = {rep: n for rep,
                        n in x_multiplicities.items() if rep != Scalar}

    def nelems(nx, rep): return min(nx, rep.size())
    ...
    def lazy_projection(params, x):
        ...
        for rep, W_mult in W_multiplicities.items():
            ...
            bilinear_params = params[i:i_end].reshape(
                W_mult, n)  # bs,nK-> (nK,bs)
            i = i_end  # (bs,W_mult,d^r) = (W_mult,n)@(n,d^r,bs)
            bilinear_elems = bilinear_params@x[...,
                                               bids].T.reshape(n, rep.size()*bs)
...
def vis(repin, repout, cluster=True):
    ...
    plt.axis('off')
    plt.savefig(f'{rep}-jax.png', bbox_inches='tight')


def scale_adjusted_rel_error(t1, t2, g):
    error = jnp.sqrt(jnp.mean(jnp.abs(t1-t2)**2))
    tscale = jnp.sqrt(jnp.mean(jnp.abs(t1)**2)) + \
        jnp.sqrt(jnp.mean(jnp.abs(t2)**2))
