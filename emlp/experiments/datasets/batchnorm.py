...
def gate_indices(sumrep):  # TODO: add support for mixed_tensors
    ...
    assert isinstance(
        sumrep, SumRep), f"unexpected type for gate indices {type(sumrep)}"
...
class TensorBN(nn.BatchNorm0D):  # TODO: add suport for mixed tensors.
    ...
    def __call__(self, x, training):  # TODO: support elementwise for regular reps
        ...
        if training:
            ...
            v = jnp.where(smask, v, ragged_gather_scatter(
                (x ** 2).mean(self.redux), self.rep))
        ...
        y = jnp.where(smask, self.gamma.value * (x - m) * F.rsqrt(v + self.eps) +
                      self.beta.value, x*F.rsqrt(v+self.eps))  # (x-m)*F.rsqrt(v + self.eps))
...
def ragged_gather_scatter(x, x_rep):
    ...
    for rep, multiplicity in x_rep.multiplicities().items():
        ...
        y.append(x_sorted[i:i_end].reshape(multiplicity, rep.size(
        )).mean(-1, keepdims=True).repeat(rep.size(), axis=-1).reshape(-1))
        i = i_end
