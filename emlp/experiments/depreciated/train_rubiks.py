...
def makeTrainer(*, network=EMLP, num_epochs=500, seed=2020, aug=False,
    ...
    with FixedNumpySeed(seed), FixedPytorchSeed(seed):
        datasets = {'train': BrokenRubiksCube2x2(
            train=False), 'test': BrokenRubiksCube2x2(train=False)}
    model = Standardize(network(
        datasets['train'].rep_in, datasets['train'].rep_out, **net_config), datasets['train'].stats)
    dataloaders = {k: LoaderTo(DataLoader(v, batch_size=min(bs, len(v)), shuffle=(k == 'train'),
                                          num_workers=0, pin_memory=False)) for k, v in datasets.items()}
    dataloaders['Train'] = dataloaders['train']
    opt_constr = objax.optimizer.Adam
    # *min(1,e/(num_epochs/10))
    def lr_sched(e): return lr*cosLr(num_epochs)(e)
    return ClassifierPlus(model, dataloaders, opt_constr, lr_sched, **trainer_config)


if __name__ == "__main__":
    Trial = train_trial(makeTrainer)
    Trial(argupdated_config(makeTrainer.__kwdefaults__, namespace=(
        emlp.solver.groups, emlp.models.datasets, emlp.models.mlp)))
