...
def makeTrainer(*, dataset=DoubleSpringPendulum, network=EMLPH, num_epochs=2000, ndata=5000, seed=2021, aug=False,
                ...
                trainer_config={'log_dir': None, 'log_args': {
                    'minPeriod': .02, 'timeFrac': .75}, },  # 'early_stop_metric':'val_MSE'},
                save=False,):
    ...
    # *cosLr(num_epochs)(e)#*min(1,e/(num_epochs/10))
    def lr_sched(e): return lr
    return IntegratedDynamicsTrainer(model, dataloaders, opt_constr, lr_sched, **trainer_config)


if __name__ == "__main__":
    Trial = hnn_trial(makeTrainer)
    cfg, outcome = Trial(argupdated_config(
        makeTrainer.__kwdefaults__, namespace=(emlp.groups, emlp.nn)))
    print(outcome)
