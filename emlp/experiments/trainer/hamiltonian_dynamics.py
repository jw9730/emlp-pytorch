...
def HamiltonianFlow(H, z0, T):
    ...
    def dynamics(z, t): return hamiltonian_dynamics(H, z, t)
...
class HamiltonianDataset(Dataset):
    ...
    def animate(self, zt=None):
        ...
        if zt is None:
            zt = np.asarray(self.integrate(
                self.sample_initial_conditions(10)[0], self.T_long))
        # bs, T, 2nd
        if len(zt.shape) == 3:
...
class IntegratedDynamicsTrainer(Regressor):
    ...
    def metrics(self, loader):
        def mse(mb): return np.asarray(self.loss(mb))
        return {"MSE": self.evalAverageMetrics(loader, mse)}

    def logStuff(self, step, minibatch=None):
        loader = self.dataloaders['test']
        metrics = {'test_Rollout': np.exp(self.evalAverageMetrics(
...
class Animation(object):
    def __init__(self, qt, lims=None, traj_lw=1, figkwargs={}):
        ...
        self.ax = self.fig.add_axes(
            [0, 0, 1, 1], projection='3d') if d == 3 else self.fig.add_axes([0, 0, 1, 1])
        ...
        if lims is None:
            lims = (min(lower), max(upper)), (min(lower),
                                              max(upper)), (min(lower), max(upper))
...
class PendulumAnimation(Animation):
    def __init__(self, qt, *args, **kwargs):
        ...
        self.objects["pts"] = sum([self.ax.plot(
            *empty, "o", ms=10, c=self.colors[i]) for i in range(self.qt.shape[1])], [])
...
class CoupledPendulumAnimation(PendulumAnimation):
    ...
    def update(self, i=0):
        ...
        x, y, z = (align2ref(diffs, self.helix) +
                   qt_padded[:-1][:, None]).reshape(-1, 3).T
...
class hnn_trial(object):
    ...
    def __call__(self, cfg, i=None):
        try:
            ...
            if i is not None:
                orig_suffix = cfg.setdefault(
                    'trainer_config', {}).get('log_suffix', '')
            ...
            for mb in trainer.dataloaders['test']:
                trajectories.append(pred_and_gt(
                    trainer.dataloaders['test'].dataset, trainer.model, mb))
            torch.save(np.concatenate(trajectories),
...
class ode_trial(object):
    ...
    def __call__(self, cfg, i=None):
        try:
            ...
            trajectories = []
            for mb in trainer.dataloaders['test']:
                trajectories.append(pred_and_gt_ode(
                    trainer.dataloaders['test'].dataset, trainer.model, mb))
            torch.save(np.concatenate(trajectories),
                       f"./{cfg['network']}_{cfg['net_config']['group']}_{i}.t")
        except Exception as e:
