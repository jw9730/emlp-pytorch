...
def rel_error(t1, t2):
    ...
    scale = jnp.sqrt(jnp.mean(jnp.abs(t1)**2)) + \
...
def parametrize(cases, ids=None):
    ...
    def decorator(test_fn):
        ...
        theids = [strip_parens(str(case))
                  for case in cases] if ids is None else ids
...
def test_sum(G):
    N = 5
    rep = T(0, 2)+3*(T(0, 0)+T(1, 0))+T(0, 0)+T(1, 1) + \
        2*T(1, 0)+T(0, 2)+T(0, 1)+3*T(0, 2)+T(2, 0)
    rep = rep(G)
...
def test_high_rank_representations(G):
    ...
    for p in range(r+1):
        for q in range(r-p+1):
            ...
            err = vmap(scale_adjusted_rel_error)(
                gv, v+jnp.zeros_like(gv), g).mean()
            if np.isnan(err):
...
def test_equivariant_matrix(G, repin, repout):
    ...
    gWerr = vmap(scale_adjusted_rel_error)(
        gvecW, W.reshape(-1)+jnp.zeros_like(gvecW), gs).mean()
