...
from emlp.reps import vis, sparsify_basis, V, Rep, T
import jax
...
def get_dsmb(dsclass):
    ...
    with FixedNumpySeed(seed), FixedPytorchSeed(seed):
        ds = dsclass(100)
    dataloader = DataLoader(ds, batch_size=min(
        bs, len(ds)), num_workers=0, pin_memory=False)
...
def test_init_forward_and_equivariance(dsclass):
    ...
    assert equivariance_err(model, mb, ds.rep_in, ds.rep_out,
                            ds.symmetry) < 1e-4, "Objax EMLP failed equivariance test"
...
def test_haiku_emlp(dsclass):
    ...
    def model(x): return net.apply(params, x)
    assert equivariance_err(model, mb, ds.rep_in, ds.rep_out,
                            ds.symmetry) < 1e-4, "Haiku EMLP failed equivariance test"
...
def test_flax_emlp(dsclass):
    ...
    def model(x): return net.apply(params, x)
    assert equivariance_err(model, mb, ds.rep_in, ds.rep_out,
                            ds.symmetry) < 1e-4, "flax EMLP failed equivariance test"
...
def test_pytorch_emlp(dsclass):
    ...
    def model(x): return jax.device_put(
        net(torch.from_numpy(np.asarray(x)).to(device)).cpu().data.numpy())
    assert equivariance_err(model, mb, ds.rep_in, ds.rep_out,
                            ds.symmetry) < 1e-4, "Pytorch EMLP failed equivariance test"


def test_utilities():
    W = V(SO(3))
    # W = V(DirectProduct(SO(3), S(6)))
    vis(W, W)
    Q = (W**2 >> W).equivariant_basis()
    SQ = sparsify_basis(Q)
    A = SQ@(1+np.arange(SQ.shape[-1]))
    nunique = len(np.unique(np.abs(A)))
    assert nunique in (SQ.shape[-1], SQ.shape[-1] +
                       1), "Sparsify failes on SO(3) T3"
